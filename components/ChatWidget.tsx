  ...
  return (
    ...
    >
      {isOpen && (
        <div 
          ref={dialogRef}
          className="mb-4 w-[calc(100vw-2rem)] sm:w-[380px] h-[75vh] sm:h-[600px] flex flex-col bg-white rounded-[24px] md:rounded-[28px] shadow-2xl overflow-hidden border border-[#dadce0] animate-in slide-in-from-bottom-4 duration-300"
          role="dialog"
          aria-modal="false"
          aria-labelledby="chat-title"
        >
          {/* Header */}
          <div className="px-5 py-4 md:p-6 bg-white border-b border-[#dadce0] flex justify-between items-center">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-[#e8f0fe] flex items-center justify-center text-[#1a73e8]" aria-hidden="true">
                <ICONS.Bot />
              </div>
              <div>
                <h2 id="chat-title" className="font-semibold text-xs md:text-sm text-[#202124]">AI Assistant</h2>
            ...
            <button 
              ...
              className="hover:bg-[#f1f3f4] p-1.5 md:p-2 rounded-full transition-colors text-[#5f6368]"
          ...
          <div 
            ref={scrollRef} 
            className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6 bg-[#ffffff] no-scrollbar"
            aria-live="polite"
            aria-atomic="false"
            role="log"
          >
            {config.availabilityStatus !== 'online' && (
              <div className="p-3 md:p-4 bg-[#fff8e1] border border-[#ffe082] rounded-2xl text-[10px] md:text-[11px] text-[#795548] font-medium leading-relaxed mb-4">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5 md:gap-2" aria-label="Reference links">
                    {m.chunks.map((chunk, idx) => chunk.web && (
                      <a 
                        ...
                        className="text-[9px] md:text-[10px] bg-[#e8f0fe] text-[#1a73e8] px-2.5 py-1.5 rounded-full border border-[#d2e3fc] hover:bg-[#d2e3fc] transition-colors flex items-center gap-1 font-medium"
                      >
            ...
            {isLoading && (
              <div className="flex justify-start" aria-label="AI is thinking...">
                <div className="bg-[#f1f3f4] px-3 md:px-4 py-2 md:py-3 rounded-[18px] md:rounded-[20px] rounded-tl-none">
          ...
          {/* Input Area */}
          <div className="px-4 py-4 md:p-6 bg-white border-t border-[#dadce0]">
            <div className="relative">
              <input
                ...
                className="w-full bg-[#f8f9fa] text-[#202124] pl-4 pr-12 md:pl-5 md:pr-14 py-2.5 md:py-3.5 rounded-full border border-[#dadce0] focus:outline-none focus:border-[#1a73e8] focus:bg-white focus:ring-1 focus:ring-[#1a73e8] transition-all text-xs md:text-sm placeholder:text-[#9aa0a6]"
              />
              <button 
                onClick={handleSend} 
      ...
      <button
        ...
        className="w-14 h-14 md:w-16 md:h-16 rounded-full bg-white text-[#1a73e8] shadow-xl flex items-center justify-center hover:scale-105 active:scale-95 transition-all group border border-[#dadce0]"
      >
        ...
        <div className={`absolute -top-0.5 -right-0.5 md:-top-1 md:-right-1 w-3.5 h-3.5 md:w-4 md:h-4 ${statusBg[config.availabilityStatus]} rounded-full border-2 border-white animate-pulse`} aria-hidden="true"></div>
      </button>
    </aside>
  );
};
